...
> Migrated from **Linode** (2 GB, `us-ord`) to **DigitalOcean**
> (`s-1vcpu-2gb`, `nyc3`). `nyc3` was chosen for low latency to Alpaca (NY-based).

## What this provisions

| Resource | Purpose | Was (Linode) |
| --- | --- | --- |
| `digitalocean_droplet.app` | 2 GB app + co-hosted Postgres box, backups on | `linode_instance` |
| `digitalocean_reserved_ip.app` + `_assignment` | Stable public IP (`RESERVED_IPV4`) | Linode reserved IPv4 |
| `digitalocean_firewall.app` | SSH/HTTP/HTTPS in, all out | `linode_firewall` |
| `digitalocean_ssh_key.deploy` | Deploy SSH key | Linode SSH key |
...
```bash
# Old (Linode):
#   linode-cli networking ip-reserve --region us-ord
# New (DigitalOcean):
...
State lives in an **S3-compatible** bucket. The existing **Linode Object
Storage** bucket keeps working unchanged (it is S3-compatible), so no forced
migration is required.

To optionally move state to **DigitalOcean Spaces** (also S3-compatible):

1. Create a Spaces bucket in `nyc3` and generate Spaces access keys.
2. Update `endpoints` in `backend-<env>.hcl` to
   `https://nyc3.digitaloceanspaces.com` and set the `TFSTATE_S3_*` secrets to
   the Spaces keys.
3. Re-init and migrate state:
   `tofu init -migrate-state -backend-config=backend-<env>.hcl`
...
Removed with the migration: `LINODE_TOKEN`, `LINODE_ROOT_PASS`.
